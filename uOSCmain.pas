...
type
  TFormuOSCR = class(TForm)
    ...
    GroupBox1: TGroupBox;
    TrackBarY: TTrackBar;
    CheckBoxTV: TCheckBox;
    CheckBoxGraph: TCheckBox;
    ...
    procedure CheckBoxGraphClick(Sender: TObject);

    procedure CheckBoxTriggerClick(Sender: TObject);
    procedure CheckBoxTVClick(Sender: TObject);
...
var
  ...
  OSCDoubleBuff: array [channel0 .. channel1] of PDoubleArray;
  ReadBytesFromOSC: array [channel0 .. channel1] of DWORD;
  fRealLength: DWORD;
  DataIsReady: Boolean;
  sample_num: Integer;
  smpArr: PDwordArray;
...
procedure TFormuOSCR.CaptureStart;
...
begin
  ...
  var
  needNewCapture := true;
  while CheckBoxGraph.Checked or CheckBoxTV.Checked do
  begin
    if needNewCapture then
    begin
      fRealLength := Capture(fCaptureLength, 0) * 1024;
      needNewCapture := False;
    end;
    Application.ProcessMessages;
    if DataIsReady then
    begin

      if CheckBoxGraph.Checked then
        OnDeviceReadData;
...
procedure TFormuOSCR.CheckBoxGraphClick(Sender: TObject);
begin
  if CheckBoxGraph.Checked then
    CaptureStart;
end;
...
procedure TFormuOSCR.CheckBoxTVClick(Sender: TObject);
begin
  if CheckBoxTV.Checked then
    CaptureStart;
